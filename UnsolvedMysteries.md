...
Can anyone tell me how to use the gcc "warning()" funtion attribute to produce a warning when a function is used? I've been trying to figure out how to do this for years, and it would let me give much more useful compiler output in the cases where user code is doing things that are clearly suspect, but not necessarily wrong? There are a bunch of places where I'd like to show a warning if they've used a function, or used it when certain compilation options were set? I'd like to be able to do what I do now with badCall() and badArg() in my cores, only with warnings instead of errors.
...
The only way I know to find these other than digging through headers hoping to find stuff accidentally left in there, is through the less-than-sophisticated process known in info-sec circles as "fuzzing". That consists of setting options that are not expected to be valid or do anything while watching for unexpected behavior, and checking whether the bits "stick" when set or not. (some bits will not even stick, implying that the hardware can't even store them. These would not be expected to be associated with undocumented features unless you were to come across a single register that contains some bits that stick and some that don't. That would indicate that *some bit* in the register does *something* if set to some value. Otherwise, we would expect an unused/inactive register to be made up of either 8 sticky bits or more likely 8 non-stick bits, but absolutly not a mix of the two.

### Reserved fuse bits
Most of these fuse bits are known to do nothing. Microchip has shipped product with them set to 0 instead of 1 at the factory; many people including Microchip's own QA deparment (if it has one *eyeroll*) didn't notice. So most of them don't do anything.

Reserved values for fuse group codes have more of a chance of being interesting. Do any of the other BOD voltage possibilities on the Dx-series do anything? The tinyAVRs have a ton of "secret" BOD voltage options which work just fine but are not documented.

What's with OSCCFG on the Dx having 4 bits, but only 2 values that do anything??? (I'm going to guess that at least some of them will be used in the DU, but will do nothing until then. They will need to configure the clock source such that USB will work if you can upload via USB without relying on a user-supplied bootloader, which the pulled product brief could have been interpreted as implying will be a feature (it's hard to say what "Immutable Boot" means, but it's certainly suggestive, no?)

### Reserved Group Codes in registers
...
There's nothing interesting in TCD0.CTRLA, the reserved count prescaler option just makes the timer stop (well, not start, since you have to turn it off to change this)

### Reserved bits in registers associated with peripherals present on the part
...
### Reserved Registers
The first two categories described where you're most likely to find active registers. If those don't give up any interesting secrets on the parts you're looking at, there's only one place left. Unfortunately, it's a big place: Registers within the extended I/O address space, yet which do not have any documented function.

Some of these will be associated with specific peripherals, and you can safely assume that they are going to relate only to that peripheral - these are the reserved bytes from the struct definition. They are also more likely to hide an undocumented feature than an arbitrary unused register address within the extended I/O space.

So, we can separate these into four categiories:
1. Registers associated with specific peripherals yet marked reserved.
2. Registers below 0x1000 that are associated with a peripheral on some parts, but this part doesn't have that peripheral. These are not very likely to be fruitful - though if you find a bonus instance of a peripheral, that was certainly worth it - if you know what might have gone there, this also makes figuring out how to use it really easy compared to a mystery register.
3. Registers between 0x0040 and 0x1000, yet which are NOT associated with a peripheral on this or similar parts,
4. Registers between 0x1000 and 0x1040 - the NVMCTRL peripheral (we also know 2 of the registers here on the EA-series - EECALB and EECALC - and what the bitfields in them do vaguely speaking - though not the group codes - though we don't know whether production parts actually have anything here)
5. Registers 0x1040 and the start of SRAM not including those containing the EEPROM, USERROW or SIGROW. These are less likely than other regions to have interesting things hiding


#### Preparation
Generate 5 lists corresponding to the 5 categories listed above for the parts you are looking at.

Be sure that none of the registers associated with the serial port you using show up in any of the lists.

For each of these categories, you should do the following:

For a reference state - which will be the state your test sketches start in (likely nothing more than a UART enabled to output serial at some baud rate) loop over all the addresses from 0x040 (0x0000 to 0x003F is the I/O space, and we pretty much know what that's about: the low half is 7 VPORTs and 4 GPIO registers, and the high half has the CCP, SPL, SPH and SREG in it, and none of the registers there are writable if they're not one of those (I already checked for unused-yet-present I/O registers ;-) - An unused I/O register that *worked* would for sure be worth free hardware (I mean, I want it so badly that I checked for them myself - and we all know that one of my guiding principles is "Never do yourself what you can trick someone else into doing for you") - if there were any to be found. But there aren't, which is a shame.) to the start of the EEPROM section. Format that into a uint8_t array and store it in progmem (if it fits). That's your reference. Most of the reserved addresses will read 0x00.

The Serial.printHex() methods are recommended here.

### Reserved registers - data gathering
From your reference known state - well... I guess I'd take the one's complement of the value  Then read it back. If you see a non-0xFF value in the address you wrote, that's a **class A** prospect. If you see 0xFF, that's a **class B** prospect. And if you see 0x00, there's no reason to belive it is writable... however, your code should also after each address, read every register in the 0x0040 to 0x1000 range mentioned above and compare it to the reference. any register that now holds a new value should be recorded (to serial). After encountering that, write the most recent address you were working with where you saw that artifact to EEPROM, do a softeware reset, and pick up where you left off.
...
```
Someone should really try it out and figure out what it does! I'd output PWM from a timer clocked from TCA0, and monitor it on the scope while triggering the event. Have the chip running something simple like blink, so if the OSCTEST event is an [HCF](https://wikipedia.org/) so you know if the event straight up freezes the chip or something. My guess is that it's and event so that they could fire it *while executing code* so they can see if whatever they're testing breaks stuff.
